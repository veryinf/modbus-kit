...
func RunTCPSlave() {
	...
	logger.Info("正在启动Modbus TCP Slave服务...")
	...
	deviceInfo := &slave.DeviceInfo{
		...
		Identification: &common.DeviceIdentification{
			...
			ProductName:         "Modbus Slave设备",
			ModelName:           "MSD-100",
			UserApplicationName: "Modbus测试应用",
		},
	}
	...
	// 设置输入寄存器初始值
	for i := 0; i < 10; i++ {
		store.Write(slave.PointTypeInputRegister, uint16(i), uint16(i*50))
	}
	...
	// 设置离散输入初始值（前10个为true，后10个为false）
	for i := 0; i < 20; i++ {
		value := uint16(0)
		if i < 10 {
			value = uint16(1)
	...
	logger.Info("Modbus TCP Slave已配置完成，正在启动服务器...")
